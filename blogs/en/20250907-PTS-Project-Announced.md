# It’s time: The Jiachen Project has launched the construction of the RISC-V open source software performance tracking platform (PTS)
...
The idea of the RISC-V open source software performance tracking platform (PTS) was first born in 2020 [1]. At that time, Mr. Wu Wei, the leader of the Jiachen Project, as the co-chair of the Code Speed SIG of the RISC-V International Foundation, hoped to build a visual system similar to Mozilla's `AreWeFastYet.com` platform to track the performance of various important open source software including JavaScript Engines on RISC-V chips and systems. At that time, the development of RISC-V IP and chips was still focused on the embedded field, and there were only a few models of development boards that could run the Linux desktop environment, such as SiFive Unmatched.
